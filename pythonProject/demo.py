...
def login():
    if request.method == 'POST':
        ...
        con = pymysql.connect(host='123.57.228.221', user='admin', password='mqc201348', database='roomsystem',
                              charset='utf8')
        cur = con.cursor()
        ...
        if result1 > 0:
            ...
            for detailrow in lst:
                scode = detailrow[2]
                if code == scode:
                    sqlSearch3 = "select * from class where CID=%s"
                    result3 = cur.execute(sqlSearch3, number)
                    cur.close()
                    con.close()
                    if result3>0:
                        return jsonify({"who": '1', "success": '1',"detail":'1' })
                    else:
                        return jsonify({"who": '1', "success": '1', "detail": '0'})
                else:
                    cur.close()
                    con.close()
                    return jsonify({"who": '1', "success": '0',"detail":'0' })
        con = pymysql.connect(host='123.57.228.221', user='admin', password='mqc201348', database='roomsystem',
                              charset='utf8')
        cur = con.cursor()
        ...
        if result2 > 0:
            ...
            for detailrow in lst:
                ...
                if code == dcode:
                    cur.close()
                    con.close()
                    return jsonify({"who": '2', "success": '1', })
                else:
                    cur.close()
                    con.close()
