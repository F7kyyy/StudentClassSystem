...
class user(object):
    ...
    def publicTask(self, title, content, opening_time, finish_time, payment):
        ...
        cursor.execute("insert into user(user_ID,title,content,opening_time,finish_time,payment) \ values('%d','%d','%s','%s','%s')" % \
                       (self.user_ID, title, content, opening_time, finish_time, payment))
        con.commit()
        print('发布成功')
        con.close()
        cur.close()
